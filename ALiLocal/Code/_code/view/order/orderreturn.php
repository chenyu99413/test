		...
		<table <?php if(request('return_id')):?>style="width: 90%"<?php else :?>style="width: 70%"<?php endif;?>>
           <tbody>
               <tr>
                   ...
                   <?php endif;?>
                   <th>阿里单号</th><td id="ali_order_no"><?php echo $return->ali_order_no?$return->ali_order_no:request('ali_order_no')?></td>
                   <th>退件范围</th>
                   <td>
                    <?php
               ...
               <tr>
                    <th>备注</th>
                    <td colspan="3">
                        <input id="remark" name='remark' type='text' style="width: 100%;height:60px" value="<?php echo $return->remark?>" placeholder=''>
                    </td>
                    <th style="color:#f00">国内退件请核对阿里单号</th>
                    <td><input type="text" id="x_als"></td>
               </tr>
           </tbody>
        </table>
...
$(function(){
	$('form').submit(function(){
		console.log($("select[name='cargo_direction']").val());
		if($("select[name='cargo_direction']").val() != 2){
			console.log(2);
			if($('#x_als').val() != $('#ali_order_no').html()){
				console.log(3);
				$.messager.alert('', '请核对阿里单号');
				return false;
			}
		}
