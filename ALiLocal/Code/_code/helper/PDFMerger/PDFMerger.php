...
class PDFMerger
{
	 ...
	 * @return PDF
	 */
	public function merge($outputmode = 'browser', $outputpath = 'newfile.pdf',$watermarkpath)
	{
				...
				for($i=1; $i<=$count; $i++)
				{
					...
					if(count($watermarkpath)>0){
					    if(isset($watermarkpath['chapter1'])){
					        $fpdi->image($watermarkpath['chapter1'], 5, 122, 39);
					    }
					    if(isset($watermarkpath['chapter2'])){
					        $fpdi->image($watermarkpath['chapter2'], 55, 122, 39);
					    }
					    if(isset($watermarkpath['chapter3'])){
					        $fpdi->image($watermarkpath['chapter3'], 32, 54, 65);
					    }
					}
				}
			}
