...
abstract class Controller_Abstract extends QController_Abstract {
	...
	function execute($action_name, array $args = array()) {
		...
		if ($this->_login_user ["staff_id"] == null && $uri != "staff/login" && $uri != "staff/logout"&& $uri != "staff/check" && $this->_context->controller_name!='api' && $this->_context->controller_name!='cron' && $this->_context->controller_name!='oa' && $this->_context->controller_name !='wxalipickupdevice' && $this->_context->controller_name !='wxapi') {
		...
		QLog::log(MyApp::currentUser("staff_name"));
		return $response;
	}
	...
	protected function _redirectAjax($success = FALSE, $message = '', $url = '', $code = '', $data = array()) {
