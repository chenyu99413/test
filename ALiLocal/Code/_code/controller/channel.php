...
class Controller_Channel extends Controller_Abstract {
	...
	function actionSearch() {
		...
		//条件搜索，是否启用
		if(request('channel_status')){
			$channel->where('channel_status=?',request('channel_status'));
		}
		$channel = $channel->getAll();
		$this->_view ['channels'] = $channel;
	...
	function actionEdit() {
		...
		if(request_is_post()){
			...
			if(!$channel_check->isNewRecord() && ($channel_check->channel_id!=request('channel_id'))){
				...
				$channel->channel_status=request('channel_status');
				$channel->save();
		...
		$this->_view['declare_threshold']=Channeldeclarethreshold::find('channel_id=?',$channel->channel_id)->getAll();
	...
	function actionDisabledsave(){
		if (request('disable_department')){
			...
			$country_group_id = explode(',',$p['country_group_id']);
			$p['country_group_id'] = $country_group_id[0];
			$disabled = Channeldeclarethreshold::find ( "id = ?", $p ["id"] )->getOne ();
			$disabled->channel_id = request('channel_id');
	...
	function actionDisableddel() {
		if (request ( "id" )) {
			//删除
			$disabled = Channeldeclarethreshold::find ( "id = ?", request ( "id" ) )->getOne ();
			$disabled->destroy ();
		}
		exit();
	}
