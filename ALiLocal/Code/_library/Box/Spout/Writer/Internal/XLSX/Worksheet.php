...
EOD;
    ...
    protected function startSheet()
    {
        ...
        fwrite($this->sheetFilePointer, self::SHEET_XML_FILE_HEADER . XLSX_EOL);
        if (is_array(self::$_WIDTHS) && count(self::$_WIDTHS)){
        		fwrite($this->sheetFilePointer, '<cols>');
        		foreach (self::$_WIDTHS as $i=> $w){
        			fwrite($this->sheetFilePointer, '<col min="'.($i+1).'" max="'.($i+1).'" width="'.$w.'" customWidth="1"/>');
        		}
        		fwrite($this->sheetFilePointer, '</cols>'.XLSX_EOL);
        }
        fwrite($this->sheetFilePointer, '    <sheetData>' . XLSX_EOL);
    }
    public static $_WIDTHS;
    ...
    public function addRow($dataRow)
    {
        ...
        $data = '        <row r="' . $rowIndex . '" spans="1:' . $numCells . '">' . XLSX_EOL;

        foreach($dataRow as $cellValue) {
            $columnIndex = CellHelper::getCellIndexFromColumnIndex($cellNumber);
            $data .= '            <c r="' . $columnIndex . $rowIndex . '"';

            if (is_numeric($cellValue) && (substr($cellValue, 0,1)!='0' || substr($cellValue, 0,2)=='0.' ) ){
            	# @firzen fixed
            	$data .= '><v>' . $cellValue . '</v></c>' . XLSX_EOL;
            }else if (CellHelper::isNonEmptyString($cellValue)) {
            	if (substr($cellValue, 0,1)=="'"){
            		$cellValue=substr($cellValue, 1);
            	}
                if ($this->shouldUseInlineStrings) {
                    $data .= ' t="inlineStr"><is><t>' . $this->stringsEscaper->escape($cellValue) . '</t></is></c>' . XLSX_EOL;
                } else {
                    $sharedStringId = $this->sharedStringsHelper->writeString($cellValue);
                    $data .= ' t="s"><v>' . $sharedStringId . '</v></c>' . XLSX_EOL;
                }
