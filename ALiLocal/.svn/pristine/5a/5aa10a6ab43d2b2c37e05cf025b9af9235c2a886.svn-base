...
class Helper_ReturnLabel_Dhl{
	...
	static function dhl($order,$channel_id){
		set_time_limit(0);
		$outs=Returnoutpackage::find('return_order_id = ?',$order->return_order_id)->getAll();
		$state='';
		//判断收件国家是否是US
		...
		//当整票出库计费重大于等于4KG，且实重小于等于3.5KG时，整票实重按照4KG推送。
		//整票实重按照4KG推送。
		$weightarr = Helper_Quote::returnreturngetweightarr($order,2,$channel);
		...
		$goods = ReturnOrderproduct::find('return_order_id = ?',$order->return_order_id)->getAll();
		$amount = 0;
		$description = '';
		$descriptioncn = '';
		$dealer = 0;
		...
		if($return['State']=='1'){
			//重发
			$order->new_tracking_no = $return['Data']['AirwayBillNumber'];
			//保存
			$order->save();
			//删除原单号
