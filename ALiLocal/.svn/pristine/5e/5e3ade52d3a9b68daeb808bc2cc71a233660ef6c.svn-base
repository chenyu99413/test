...
class ChinesePinyin{
	...
	public function __construct(){
		if( empty($this->ChineseCharacters) ){
		  $this->ChineseCharacters = file_get_contents(dirname(__FILE__).DS.'ChineseCharacters.dat');	
		}
	}
	...
	*/	
	...
	/**
	 * @todo   转成带无声调的汉语拼音(首字母大写)
	 * @author stt
	 * @since  2020-11-05
	 * @link   #83311
	 */
	public function transformWithoutToneucfirst($input_char,$delimiter='',$outside_ignore=true){
		//首字母大写
		$char_with_tone = $this->transformWithToneucfirst($input_char,$delimiter,$outside_ignore);
		//首字母大写
		$char_without_tone  =  str_replace(array('ā','á','ǎ','à','ō','ó','ǒ','ò','ē','é','ě','è','ī','í','ǐ','ì','ū','ú','ǔ','ù','ǖ','ǘ','ǚ','ǜ','ü'),
			array('a','a','a','a','o','o','o','o','e','e','e','e','i','i','i','i','u','u','u','u','v','v','v','v','v')
			,$char_with_tone );
	 ...
	 * @link   #83311
	 */
	public function transformWithToneucfirst($input_char,$delimiter=' ',$outside_ignore=false){
		$input_len = mb_strlen($input_char,$this->charset);
		$output_char = '';
		for($i=0;$i<$input_len;$i++){
			$word = mb_substr($input_char,$i,1,$this->charset);
			...
			if(preg_match('/^[\x{4e00}-\x{9fa5}]$/u',$word) && preg_match('/\,'.preg_quote($word).'(.*?)\,/',$this->ChineseCharacters,$matches) ){
				//首字母大写
				$output_char.=ucfirst($matches[1].$delimiter);
			}else if(!$outside_ignore){
				$output_char.=$word;
			}
		}
