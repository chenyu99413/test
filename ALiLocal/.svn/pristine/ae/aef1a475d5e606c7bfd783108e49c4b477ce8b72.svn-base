...
<form method="POST">
	...
	<table class="FarTable">
		...
		<?php $state=array('0'=>'未抵达','1'=>'已抵达'); $i=1; foreach ($lists as $list): 
		      $total_id = Totaloutlist::find('total_no = ?',$list->total_no)->getOne();
		?>
			<tr>
				<td><?php echo $i++ ?></td>
				<td><?php echo $state[$list->state] ?></td>
				<td>
    				<a target="_blank" href="<?php echo url('warehouse/edittatolout', array('total_id' => $total_id->total_id))?>">
            		  <?php echo $list->total_no?>
    				</a>
				</td>
