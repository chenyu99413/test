				...
				<table id="table_remote" class="FarTable">
					...
					<tbody>
						...
						<tr id="<?php echo $value->remote_id?>">
							...
							<td style="text-align: right"><?php echo $value->currency_code?></td>
							<td>
								<a class="btn btn-mini" href="javascript:void(0);"
									onclick="EditRow([{'type':'combogrid','option':'country','url':'<?php echo url('common/countrygrid')?>','checkUrl':'<?php echo url('common/checkcountryexist')?>','required':'true'},{'type':'text'},{'type':'text'},{'type':'text'},{'type':'number','precision':'2','min':'0'},{'type':'number','precision':'2','min':'0'},{'type':'number','precision':'2','min':'0'},{'type':'number','precision':'2','min':'0'},{'type':'number','precision':'2','min':'0'},{'type':'select','option':<?php echo str_replace("\"","'",json_encode(CodeCurrency::getCurrencyList()));?>}],this);">
		...
		$.ajax({
			...
			data:{"id":"<?php echo request('id')?>",
				  "remote":{
					...
					"remote_city":$(obj).children().eq(1).text(),
					"start_postal_code":$(obj).children().eq(2).text(),
					"end_postal_code":$(obj).children().eq(3).text(),
					...
					"currency_code":$(obj).children().eq(9).text()}},
			success:function(msg){
				if(msg == '币种不存在'){
					alert(msg)
				}else{
