        ...
        if (is_array($source)) {
            ...
            foreach ($source as $rowData) {
                ...
                foreach($rowData as $cellValue) {
                    if ($strictNullComparison) {
                        if ($cellValue !== $nullValue) {
                            ...
                            if (substr($cellValue, 0,1)=="'"){
                            	if (strlen($cellValue) ==1){
                            		$cellValue=' ';
                            	}else {
                            		$cellValue=substr($cellValue, 1,strlen($cellValue)-1);
                            	}
                            	$this->getCell($currentColumn . $startRow)->setValueExplicit($cellValue,PHPExcel_Cell_DataType::TYPE_STRING2);//->setValue($cellValue);
                            	$this->getStyle($currentColumn . $startRow)->setQuotePrefix(true);
                            }else {
                        ...
                        if ($cellValue != $nullValue) {
                        	...
                        	if (substr($cellValue, 0,1)=="'"){
                        		...
                        		$this->getCell($currentColumn . $startRow)->setValueExplicit($cellValue,PHPExcel_Cell_DataType::TYPE_STRING2);
                        		$this->getStyle($currentColumn . $startRow)->setQuotePrefix(true);
                        	}else {
                        		$this->getCell($currentColumn . $startRow)->setValue($cellValue);
                    ...
                    $this->getStyle($currentColumn . $startRow)->applyFromArray($styleArray);
                    $this->getColumnDimension($currentColumn)->setAutoSize(true);
