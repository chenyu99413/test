		  ...
		  <?php $order_count=0;$weight_cost=0;$weight_actual=0;$quantity=0;$quantity_sum=0;
			...
			<tr>
				...
				<td><?php if($temp->status == '0'):?>
				    ...
				    <?php endif;?>
				    <?php endif;?>
				    <?php if($temp->type == '0' && MyApp::checkVisible('totalout-ture')):?>
				    <input type="button" class="btn btn-small btn-primary" href="javascript:void(0)" data="<?php echo $temp->total_id?>" onclick="queren(this)" value="确认">
				    <?php endif;?>
				</td>
...
function queren(obj){
	if (confirm('是否确认?')) {
    	var total_id=$(obj).attr('data');
    	$.ajax({
    		url:'<?php echo url('warehouse/truetotalout')?>',
    		data:{total_id:total_id},
    		type:'post',
    		async:false,
    		success:function(data){
