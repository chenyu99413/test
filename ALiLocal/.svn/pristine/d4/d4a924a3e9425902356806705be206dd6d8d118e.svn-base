...
class Helper_Label_Ems{
	 ...
	 * @link   #82496
	 */
	static function ems($order,$rorder = 0){
		...
		$log=new OrderLog(array(
			...
			//json
			'comment'=>'EMS订单报文：'.json_encode($arr)
		));
		$log->save();
		...
		if($return['ack']=='true'){
			...
			try {
				...
				if($rorder !== 0){
					//重发
					$rorder->new_tracking_no = $return['tracking_number'];
					$rorder->ems_order_id=$return['order_id'];
					//保存
					$rorder->save();
				}else{
					//非退件订单
