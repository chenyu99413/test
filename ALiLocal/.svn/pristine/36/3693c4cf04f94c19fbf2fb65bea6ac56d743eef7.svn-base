<?php
class Helper_ReturnLabel_Kingspeed{
	/**
	 * @todo   Kingspeed账号渠道
	 * @author stt
	 * @since  2020-09-09
	...
	static function kingspeed($order,$channel_id){
		set_time_limit(0);
		$out=Returnoutpackage::find('return_order_id = ?',$order->return_order_id)->getSum('quantity');
		if($out > 1){
		...
		$faroutpackage = Returnoutpackage::find('return_order_id = ?',$order->return_order_id)->getOne();
		$wares = ReturnOrderproduct::find('return_order_id = ?',$order->return_order_id)->getAll();
		...
		$boxes[]=array(
			'code' => '1',
			'weight' => $faroutpackage->weight<0.454?0.454:$faroutpackage->weight,
			"width" => $faroutpackage->width,
			"length" => $faroutpackage->length,
			"height" => $faroutpackage->height,
		...
		if(is_null($result)){
			...
			if($result['success']==true){
				//保存发件人抬头到订单表
				$order->new_tracking_no = $result['trackingNumber'];
